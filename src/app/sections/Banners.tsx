...
export default function Banners({ openBookingModal }: { openBookingModal?: () => void }) {
  return (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {banners.map((b, i) => (
            <motion.div key={i} {...fadeInUp} transition={{ ...fadeInUp.transition, delay: i * 0.1 }}
              ...
              className={`bg-gradient-to-br ${b.color} rounded-2xl p-6 text-white shadow-lg hover:shadow-2xl transition-all relative overflow-hidden cursor-pointer group`}>
              ...
              <div className="flex items-center gap-2 text-white/50 text-[9px] mt-1">
                <MapPin className="w-3 h-3" /> Vizag · Vizianagaram · Vissannapeta
              </div>
            </motion.div>
          ))}
        </div>
